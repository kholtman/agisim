...
function eutil(depth,state,timetochange,usofar,mod,replaylist, u,i,ucargainbase,uecargainbase,maxu,lg,lgbase,l,corr,statesig,replay,action,t,u1,u2,lgr1,modifybonus)
{
    ...
    # 200 utility function favors doing nothing: gives negative score to any car producing action
 ...
 # correction factors like the f_c function value into corr.
    ...
    if( (!replay&&(state==0)&&(timetochange<=0)) || (replay&&(action=="#")) )
    {
  ...
  # return is maximum depth reached
  ...
  # code that implements different actions  
	...
	actiontaken[depth]="E";
	u=uecargainbase*t+corr+eutil(depth+1,state,timetochange,usofar+uecargainbase*t+corr,mod,replaylist);
	l=lgbase "E" lg_return;
    ...
    if((replay&&(action=="0"))||plain_nop)
    {
	# plain do-nothing action
	actiontaken[depth]="0";
	u=corr+eutil(depth+1,state,timetochange,usofar+corr,mod,replaylist);
	...
	u=ucargainbase*(1-lobbycost)+corr+					\
	    eutil(depth+1,state,timetochange+direction,usofar+ucargainbase*(1-lobbycost)+corr,mod,replaylist);   
	...
	u=corr+eutil(depth+1,state,timetochange+direction,usofar+corr,mod,replaylist);   
